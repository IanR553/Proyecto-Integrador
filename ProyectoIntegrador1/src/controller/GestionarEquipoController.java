...
public class GestionarEquipoController {
	...
	@FXML
	private TableColumn<Equipo, String> colMarca;
	@FXML
	private TableColumn<Equipo, String> colSoftware;
	...
	public void initialize() {
		...
		colEstado.setCellValueFactory(data -> new javafx.beans.property.SimpleStringProperty(
				data.getValue().isEstado() ? "Disponible" : "No disponible"));
		colMarca.setCellValueFactory(
				data -> new javafx.beans.property.SimpleStringProperty(data.getValue().getMarca()));
		...
		tableEquipos.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
			if (newSelection != null)
				mostrarDatosEquipo(newSelection);
		});
	}
	...
	private void actionCrearEquipo() {
		...
		Equipo equipo = new Equipo(comBoxTipo.getValue(), chkEstado.isSelected(), txtMarca.getText().trim(),
	...
	private void actionActualizarEquipo() {
		...
		Equipo equipo = new Equipo(seleccionado.getId(), comBoxTipo.getValue(), chkEstado.isSelected(),
	...
	private void actionEliminarEquipo() {
		...
		if (equipoSeleccionado == null) {
			Main.showAlert("Debe seleccionar un equipo en la tabla para eliminar.", "Sin selección",
	...
	private boolean validarCampos() {
		if (comBoxTipo.getValue() == null || txtMarca.getText().trim().isEmpty()) {

			Main.showAlert("Por favor, complete los campos Tipo y Marca sin espacios en blanco.", "Campos obligatorios",
					Alert.AlertType.WARNING);
			return false;
		}
		return true;
